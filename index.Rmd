...
hotel_mngt <- hotels %>%
  ...
  select(hotel, arrival_date, arrival_date_week_number,arrival_date_month, arrival_date_year, lead_time,
         ...
         reservation_status, reservation_status_date, customer_type, market_segment, adr, is_repeated_guest) %>%
  # Tweaking a bit
...
Booking and ADR by months, if possible market_segment, customer type

```{r season}
season <- hotel_mngt %>%
  select(hotel, arrival_date_month, reservation_status, customer_type, market_segment, adr) %>%
...
p3 + stat_summary(fun.y=mean, colour="red", geom="line", aes(group = 2))

# ADR and guests by months
  
```

Would be nice if we can check the occupancy rate also 
...
NOTE: REMEMBER TO GROUP BY WEEK NUMBER RATHER THAN YEAR DUE TO INCONVENTIONAL YEAR COUNT. 
